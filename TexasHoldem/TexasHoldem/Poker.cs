    ...
    public class Poker
    {
        ...
        public static (string type, string[] ranks) Hand(string[] holeCards, string[] communityCards)
        {
            ...
            if (groups.Where(s => s.StraightFlush.Count() >= 5).Count() > 0)
                ...
                return ("four-of-a-kind", new string[]
                {
            ...
            else if (groups.Where(s => s.Ranks.Count() == 3).Count() > 0 && groups.Where(s => s.Ranks.Count() == 2).Count() > 0)
                return ("full house", new[]{
                    groups.Where(s => s.Ranks.Count() == 3).First().Card.Item2,
                ...
                return ("three-of-a-kind", new[]{
                    groups.Where(s => s.Ranks.Count() == 3).First().Ranks.Select(s => s.Item2).First(),
                ...
                return ("two pair", new[]{
                    groups.Where(s => s.Ranks.Count() == 2).Select(s => s.Card.Item2).Distinct().First(),
                    groups.Where(s => s.Ranks.Count() == 2).Select(s => s.Card.Item2).Distinct().Skip(1).First(),
                    groups.Where(s => s.Ranks.Count() == 1).First().Card.Item2
                });
